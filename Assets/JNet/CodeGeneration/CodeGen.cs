...
namespace JNetworking.CodeGeneration
{
    public static class CodeGen
    {
        ...
        private static readonly Dictionary<Type, (Process read, Process write)> processors = new Dictionary<Type, (Process, Process)>()
        {
        ...
        public static string GenBehaviourClass(Type t)
        {
            ...
            foreach (var field in fields)
            {
                if (field.GetCustomAttribute<SyncVarAttribute>() != null)
                {
                    if (!field.IsPublic)
                    {
                        JNet.Error(string.Format("SyncVar property '{0}' in {1} is not public. Syncvars must be public, non static and of a valid type.", field.Name, field.DeclaringType.Name));
                        continue;
                    }
